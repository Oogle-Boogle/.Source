...
public class Dueling {
	...
	public void challengePlayer(Player playerToDuel) {
		...
		if(player.getGameMode() == GameMode.GROUP_IRONMAN) {
			player.getPacketSender().sendMessage("Group_Ironman players are not allowed to stake.");
			return;
		}
		...
		if (Misc.getMinutesPlayed(player) <= 30) {
			player.sendMessage("You need to have played atleast 30 minutes to duel.");
			return;
		}
		if(player.getGameMode() == GameMode.HARDCORE_IRONMAN) {
			player.getPacketSender().sendMessage("Hardcore-Ironman-players are not allowed to stake.");
			return;
		}
