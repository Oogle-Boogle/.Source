package com.platinum.world.content.minigames.impl;

import com.platinum.engine.task.TaskManager;
import com.platinum.engine.task.impl.CeilingCollapseTask;
		...
		case 10284:
			...
			if (player.getMinigameAttributes().getBarrowsMinigameAttributes().getBarrowsData()[player.getMinigameAttributes().getBarrowsMinigameAttributes().getRandomCoffin()][1] == 0) {
				...
				if(player.getInventory().getFreeSlots() < 4) {
					player.getPacketSender().sendMessage("You need at least 4 free inventory slots to loot this chest.");
					return true;
				}
				resetBarrows(player);
				...
				if (Misc.getRandom(100) >= 84) {
					int b = randomBarrows();
					if (player.getRights().isHighDonator() || player.getRights() == PlayerRights.SUPPORT || player.getRights() == PlayerRights.MODERATOR || player.getRights() == PlayerRights.ADMINISTRATOR || player.getRights() == PlayerRights.OWNER || player.getRights() == PlayerRights.DEVELOPER ) {
					...
					World.sendMessageNonDiscord("@blu@[BARROWS] @bla@"+player.getUsername()+" has just received "+ItemDefinition.forId(b).getName()+" from the Barrows!");
				}
