...
import com.platinum.engine.task.impl.FireImmunityTask;
import com.platinum.engine.task.impl.OverloadPotionTask;
...
public class Consumables {
	...
	public static void handlePotion(final Player player, final int itemId, final int slot) {
		...
		if (player.getPotionTimer().elapsed(900)) {
			...
			case 125:
				...
				player.getInventory().delete(229, 1); player.sendMessage("@blu@We removed the vial for you!");
				break;
				/*
				 * Defence potions
				 */
			...
			case 15300:
				...
				if (player.lastSpecialRestoreClaim > System.currentTimeMillis()) {
					...
					player.sendMessage("You still need to wait another <col=8C33FF>"
							+ player.getLongDurationTimer(player.lastSpecialRestoreClaim));
					return;
				}
				player.performAnimation(new Animation(829));
				...
				player.lastSpecialRestoreClaim = System.currentTimeMillis() + 1800000;
				break;
			case 15301:
				if(player.getLocation() != null && player.getLocation() == Location.WILDERNESS) {
					player.getPacketSender().sendMessage("This potion cannot be used in the Wilderness.");
	...
	public static void drinkProPotion(final Player player, int potion, int replacePotion, int slot, int skill, boolean super_pot) {
		if(slot >= 0) {
			player.performAnimation(new Animation(829));
			player.getInventory().getItems()[slot] = new Item(replacePotion, 1);
		}
		...
		boolean cbPot = potion == 9739 || potion == 9741 || potion == 9743 || potion == 9745;
		Skill sk = Skill.forId(skill);
		player.getSkillManager().setCurrentLevel(sk, player.getSkillManager().getCurrentLevel(sk) + getBoostedmoreStat(player, skill, super_pot, cbPot), true);
	}
