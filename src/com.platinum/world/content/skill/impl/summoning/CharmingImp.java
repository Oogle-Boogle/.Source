package com.platinum.world.content.skill.impl.summoning;
...
public class CharmingImp {
	...
	public static final int GREEN_CHARM = 537;
	public static final int GOLD_CHARM = 19864;
	public static final int CRIM_CHARM = 989;
	public static final int BLUE_CHARM = 15243;
	...
	public static void sendConfig(Player player) {
		for(int i = 0; i < 4; i++) {
			...
			switch(state) {
			case 0:
				player.getPacketSender().sendMessage("<img=11> <col=996633>Your Looting Bag is placing all "+ItemDefinition.forId(charm).getName()+"s it finds in your inventory.");
				break;
			case 1:
				player.getPacketSender().sendMessage("<img=11> <col=996633>Your Looting Bag is turning all "+ItemDefinition.forId(charm).getName()+"s it finds into Summoning exp.");
				break;
			}
		}
	}
