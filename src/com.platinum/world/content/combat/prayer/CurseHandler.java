package com.platinum.world.content.combat.prayer;
...
public class CurseHandler {
	...
	private static final double getDrain(Player player) {
		...
		if(player.getInventory().contains(5170)) {
			toRemove = 0;
		}
		
		if(player.getEquipment().get(Equipment.HEAD_SLOT).getId() == 18998) {
			toRemove *= 0.25;
		}
	...
	public enum CurseData {
		...
		LEECH_ENERGY(84, 0.5, 32533, 625),
		LEECH_SPECIAL_ATTACK(86, 1, 32535, 626),
		WRATH(89, 0.6, 32537, 627),
		SOUL_SPLIT(92, 0.8, 32539, 628),
		TURMOIL(95, 0.7, 32541, 629, new PrayerAnimation(new Animation(12565), new Graphic(2226)));
