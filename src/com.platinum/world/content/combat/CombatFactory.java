...
import com.platinum.engine.task.impl.CombatSkullEffect;
...
import com.platinum.model.GraphicHeight;
...
import com.platinum.model.movement.PathFinder;
...
import com.platinum.world.content.ItemDegrading;
import com.platinum.world.content.ItemDegrading.DegradingItem;
import com.platinum.world.content.combat.effect.CombatPoisonEffect;
import com.platinum.world.content.combat.effect.CombatPoisonEffect.PoisonType;
import com.platinum.world.content.combat.effect.EquipmentBonus;
import com.platinum.world.content.combat.magic.CombatAncientSpell;
...
public final class CombatFactory {
	...
	public static int calculateMaxDragonFireHit(Character e, Character v) {
		...
		if(e.isNpc() && v.isPlayer()) {
			...
			if(victim.getFireImmunity() > 0 || victim.getEquipment().getItems()[Equipment.SHIELD_SLOT].getId() == 1540 || victim.getEquipment().getItems()[Equipment.SHIELD_SLOT].getId() == 11613 || victim.getEquipment().getItems()[Equipment.SHIELD_SLOT].getId() == 11283) {
	...
	public static boolean checkHook(Character entity, Character victim) {
		...
		if(victim.isNpc() && entity.isPlayer()) {
			...
			if(Nex.nexMob(npc.getId()) || npc.getId() == 6260 || npc.getId() == 6261 || npc.getId() == 6263 || npc.getId() == 6265 || npc.getId() == 6222 || npc.getId() == 6223 || npc.getId() == 6225 || npc.getId() == 6227 || npc.getId() == 6203 || npc.getId() == 6208 || npc.getId() == 6204 || npc.getId() == 6206 || npc.getId() == 6247 || npc.getId() == 6248 ||  npc.getId() == 6252) {
			...
			if (npc.getId() == 175 && !GameSettings.EventArena && entity.getLocation() != Location.DUNGEON_MINIGAME) {
	...
	protected static void applyPrayerProtection(CombatContainer container, CombatBuilder builder) {
		...
		if(builder.getVictim().isPlayer()) {
			...
			if(victim.getEquipment().getItems()[Equipment.SHIELD_SLOT].getId() == 6198 || victim.getEquipment().getItems()[Equipment.SHIELD_SLOT].getId() == 5211  || victim.getEquipment().getItems()[Equipment.SHIELD_SLOT].getId() == 5210 || victim.getEquipment().getItems()[Equipment.SHIELD_SLOT].getId() == 15045 || victim.getEquipment().getItems()[Equipment.SHIELD_SLOT].getId() == 13742 || victim.getEquipment().getItems()[Equipment.SHIELD_SLOT].getId() == 13744) {
				...
				TaskManager.submit(new Task(1, p, false) {
					...
					public void execute() {
						if (!(attacker == null || target == null || attacker.getConstitution() <= 0)) {
							...
							p.getSkillManager().updateSkill(Skill.CONSTITUTION);
							if (p.getRights() == PlayerRights.EXTREME_DONATOR
									|| p.getRights() == PlayerRights.LEGENDARY_DONATOR
									|| p.getRights() == PlayerRights.VIP_DONATOR
									|| p.getRights() == PlayerRights.DELUXE_DONATOR
									|| p.getRights() == PlayerRights.UBER_DONATOR) {
								if (Misc.inclusiveRandom(0, 25) > 20) {
									// heal again
									p.getSkillManager().setCurrentLevel(Skill.CONSTITUTION,
											p.getSkillManager().getCurrentLevel(Skill.CONSTITUTION) + form);
									p.getSkillManager().updateSkill(Skill.CONSTITUTION);
