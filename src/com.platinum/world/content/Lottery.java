...
public class Lottery {
	 ...
	 */
	private static final int PRICE_TO_ENTER = 100;
	...
	public static final int getPot() {
		...
		return (CONTESTERS.size() * (PRICE_TO_ENTER - 250));
	}
	...
	public static void enterLottery(Player p) {
		...
		if(p.getInventory().getAmount(10835) < PRICE_TO_ENTER && !usePouch || p.getRights() == PlayerRights.ADMINISTRATOR || p.getRights() == PlayerRights.OWNER) {
		...
		if(usePouch) {
			...
			p.getInventory().delete(10835, PRICE_TO_ENTER);
	...
	public static void restartLottery() {
		...
		try {
			...
			if(winner != null) {
				...
				writer.write("");
				writer.close();
				World.sendMessageNonDiscord("<col=D9D919><shad=0>This week's lottery winner is "+winner+"! Congratulations!");
			} else
				World.sendMessageNonDiscord("<col=D9D919><shad=0>The lottery needs some more contesters before a winner can be selected.");
		} catch(Exception e) {
			e.printStackTrace();
		}
	}
