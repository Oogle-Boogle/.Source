...
public class Scoreboards {
	...
	public static Scoreboard TOP_NPC_KILLCOUNT;
	...
	public static void init() {
		TOP_PKERS = new Scoreboard("Arlania's Top Pkers", "top-pkers.txt") {
			@Override
		...
		TOP_NPC_KILLCOUNT = new Scoreboard("Platinum's Top Npc Killcount", "top-killstreaks.txt") {
			@Override
			public void resort(ArrayList<ScoreboardEntry> entries) {
		...
		TOP_ACHIEVER = new Scoreboard("Arlania's Top Achiever", "top-achievers.txt")  {
			@Override
			public void resort(ArrayList<ScoreboardEntry> entries) {
				Scoreboards.resort(entries, "integer");
			}
		...
		TOP_TOTAL_EXP = new Scoreboard("Arlania's Top Total Exp", "top-exp.txt")  {
			@Override
			public void resort(ArrayList<ScoreboardEntry> entries) {
	...
	public static ArrayList<ScoreboardEntry> getUpdatedList(Scoreboard scoreboard) {
		...
		for(Player p : World.getPlayers()) {
			...
			} else if(scoreboard == Scoreboards.TOP_NPC_KILLCOUNT) {
				int sum = p.getKcSum();
				entries.add(new ScoreboardEntry(p.getUsername(), new String[]{String.valueOf(sum)}));
	...
	public static void open(Player player, Scoreboard scoreboard) {
		...
		for (int i = 0; i <= 50; stringId++, i++) {
			...
			String line = "";
			if(entry != null) {
				if(scoreboard == Scoreboards.TOP_NPC_KILLCOUNT) {
					line = "@whi@Rank @or1@"+(i+1)+"@whi@ - "+entry.player.replaceAll("_", " ")+" - Killcount: @or1@"+Integer.parseInt(entry.value[0]);
