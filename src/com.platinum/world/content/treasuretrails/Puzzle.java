...
public class Puzzle {// todo maybe hovering button message
	...
	public static void loadPuzzleItems(Player player) {
		...
		if(player.puzzleStoredItems == null)
			player.puzzleStoredItems = new Item[ClueScroll.PUZZLE_LENGTH];
		for (int i = 0; i < ClueScroll.PUZZLE_LENGTH; i++) {
	...
	public static void loadPuzzle(Player player) {
		player.getPacketSender().sendInterfaceItems(ClueScroll.PUZZLE_INTERFACE_CONTAINER, player.puzzleStoredItems);
		player.getPacketSender().sendInterfaceItems(ClueScroll.PUZZLE_INTERFACE_DEFAULT_CONTAINER, getDefaultItems());
		player.getPacketSender().sendInterface(ClueScroll.PUZZLE_INTERFACE);
