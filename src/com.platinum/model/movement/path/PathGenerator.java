package com.platinum.model.movement.path;
...
public static Position getCombatPath(Character character, Character following) {
		...
		if (PathFinder.accessable(followX, followY, characterZ, x3, y3) && isInteractionPathClear(x3, y3, characterZ, followX, followY)) {
	...
	public static boolean isInteractionPathClear(final int x0, final int y0, final int characterZ, final int followX, final int followY) {
		...
		if (!PathFinder.accessable(x0, y0, characterZ, followX, followY)) {
			return false;
		}
