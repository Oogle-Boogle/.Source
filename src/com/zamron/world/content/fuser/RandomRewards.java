...
public class RandomRewards {
    ...
    public static void giveReward(Skill skill, Player player) {
        ...
        if (doubleReward) {
            ...
            World.sendMessageNonDiscord("<img=11><shad=20><col=b96900>"+player.getUsername() + " Has just received double " +amountToGive / 2+ " " + ItemDefinition.forId(stoneID).getName()+plural);
        }
        ...
        player.sendMessage("You have received @blu@"
                ...
                + plural
                + "@bla@!");
            World.sendMessageNonDiscord("<img=11><shad=20><col=b96900>"+player.getUsername() + " Has just received " +amountToGive+ " " + ItemDefinition.forId(stoneID).getName()+plural);
    }
}
