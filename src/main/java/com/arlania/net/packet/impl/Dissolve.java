...
public class Dissolve {
	...
	public static enum DissItem {
		...
		DissItem(int startItem, Item[] reqItem, int chance, int EndItem) {
		...
		public static boolean checkReq(Player p, int itemId) { // test it and let me see
			for (DissItem item : DissItem.values()) {
	...
	public static void init(Player p, int itemId) {
		...
		if (DissItem.checkReq(p, itemId)) {
			p.getInventory().delete(itemId, 1);
			int random = Misc.inclusiveRandom(1, DissItem.getRandom(itemId));
			int tries = 1;
			if (random == 1) {
				p.getPacketSender().sendMessage("" + random);
				if (!ItemDefinition.forId(itemId).getName().toLowerCase().contains("monster")) {
					World.sendMessage("<shad=0>@bla@[@gr3@Upgrade@bla@]@gr3@ " + p.getUsername()
		...
		DissItem.getReqs(p, itemId);
		}


	public static boolean getItems(Player p, int id) {
		return DissItem.checkItem(p, id) != 0;
	}
}