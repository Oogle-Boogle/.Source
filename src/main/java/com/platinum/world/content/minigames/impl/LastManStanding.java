...
import com.platinum.world.content.skill.SkillManager.Skills;
...
public class LastManStanding {
	...
	public static void sequence() {
		...
		if(waitTimer > 0) { 
			...
			if (waitTimer % 100 == 0 && waitTimer > 0)
				World.sendMessage("@blu@[LAST MAN STANDING] "+waitTimer / 100 +" minutes until Last Man Standing starts!" +" Join now @ ::lms");
		}
		...
		if(waitTimer <= 0 && playersInLobby < 2 && !gameRunning ) {
			World.sendMessage("@blu@[LAST MAN STANDING]@red@ Not enough players, so the timer has been reset.");
			World.sendMessage("@blu@[LAST MAN STANDING]@red@ ::lms to join the lobby.");
			waitTimer = 300;
		}
	}
	...
	public static void startEvent() {
		if (!eventRunning) {
			eventRunning = true;
			World.sendMessage("@red@[LAST MAN STANDING]@blu@ Event has started, ::lms to join!");
		}
	}
