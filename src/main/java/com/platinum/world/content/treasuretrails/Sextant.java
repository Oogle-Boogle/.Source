...
/**
 * Created by IntelliJ IDEA. User: levi Date: 11/14/16 Time: 21:03 To change
...
public class Sextant {
	...
	public static void calculateStartFormula(Player c) {
		...
		double randomBarDegree = -BAR_LIMIT_RIGHT
				+ (c.rotationFactor / DEGREE_FACTOR)
				+ Misc.getRandom((int) (BAR_LIMIT_RIGHT
						+ (c.rotationFactor / DEGREE_FACTOR) + BAR_LIMIT_LEFT + (c.rotationFactor / DEGREE_FACTOR)));
		// sextant sun randomly placed
		int limitUpSun = (int) (BAR_LIMIT_LEFT
				+ (c.rotationFactor / DEGREE_FACTOR) - randomBarDegree);
		int limitDownSun = Math
				.abs((int) (randomBarDegree + BAR_LIMIT_RIGHT + (c.rotationFactor / DEGREE_FACTOR)));
		int incrementationSun = (Misc.getRandom(1) == 0) ? Misc
				.getRandom(limitDownSun) * 2 : -Misc.getRandom(limitUpSun) * 2;
		// sextant landscape randomly placed
		double randomLandScapeDegree = -(GLOBAL_LIMIT_RIGHT / 5.7)
				+ Misc.getRandom((int) ((GLOBAL_LIMIT_RIGHT / 5.7) + (GLOBAL_LIMIT_LEFT / 5.7)));
		int limitUpLandScape = (int) ((GLOBAL_LIMIT_LEFT / 5.7) - randomLandScapeDegree);
		int limitDownLandScape = Math
				.abs((int) (randomLandScapeDegree + GLOBAL_LIMIT_RIGHT / 5.7));
		int incrementationLandScape = (Misc.getRandom(1) == 0) ? Misc
				.getRandom(limitDownLandScape) : -Misc.getRandom(limitUpLandScape);
	...
	public static void initializeRandomSextantInterface(Player c) {
		...
		c.getPacketSender().sendInterface(SEXTANT_INTERFACE);
		updateSextant(c);
	}
	...
	public static boolean everythingMatches(Player c) {
		if (Math.abs(c.sextantLandScapeCoords) > 7) {
			c.getPacketSender()
					.sendMessage(
							"You need to get the horizon in the middle of the eye piece.");
			return false;
		}
		if (c.sextantSunCoords != 0) {
			c.getPacketSender().sendMessage(
					"You need to get the sun in the middle of the eye piece.");
			return false;
		}
		if (!c.getInventory().contains(CoordinateScrolls.SEXTANT)
				|| !c.getInventory().contains(
						CoordinateScrolls.CHART)) {
			c.getPacketSender()
					.sendMessage(
							"You need a watch and navigator's chart to work out your Position.");
			return false;
	...
	public static void moveSextantBar(Player c, boolean left) {
		...
		boolean bool1 = c.sextantBarDegree > BAR_LIMIT_LEFT
				+ (c.rotationFactor / DEGREE_FACTOR);
		boolean bool2 = c.sextantBarDegree < -BAR_LIMIT_RIGHT
				+ (c.rotationFactor / DEGREE_FACTOR);
		if (!bool1 && !bool2)
			moveSun(c, left);
		if (bool1)
			c.sextantBarDegree = BAR_LIMIT_LEFT
					+ (c.rotationFactor / DEGREE_FACTOR);
		if (bool2)
			c.sextantBarDegree = -BAR_LIMIT_RIGHT
					+ (c.rotationFactor / DEGREE_FACTOR);
		updateSextant(c);
	...
	public static void moveSextantBarWithFormula(Player c, double degreeGiven) {
		double degree = degreeGiven < -BAR_LIMIT_RIGHT
				+ (c.rotationFactor / DEGREE_FACTOR) ? -BAR_LIMIT_RIGHT
				+ (c.rotationFactor / DEGREE_FACTOR)
				: degreeGiven > BAR_LIMIT_LEFT
						+ (c.rotationFactor / DEGREE_FACTOR) ? BAR_LIMIT_LEFT
						+ (c.rotationFactor / DEGREE_FACTOR) : degreeGiven;
		int realDegree = degree > 0 ? (int) (degree * DEGREE_FACTOR)
				: 2047 - (int) (-degree * DEGREE_FACTOR);
		int x = (int) Math.round(Math.abs((SEXTANT_BAR_LENTH / 2)
				* Math.sin(degree * Math.PI / 180)));
		int y = (int) Math.floor(Math.abs((SEXTANT_BAR_LENTH / 2)
				* (1 - Math.cos(degree * Math.PI / 180))));
		c.getPacketSender().sendInterfaceEdit(SEXTANT_BAR, SEXTANT_ROTATION1,
				realDegree, SEXTANT_BAR_ZOOM);

		c.getPacketSender().sendInterfaceComponentMoval(
				degree > 0 ? -(x - c.rotationFactor / 2)
						: (x + c.rotationFactor / 2), -y, SEXTANT_BAR);
	...
	public static void moveGlobalPiece(Player c, boolean left) {
		...
		if (left) {
			...
			if (c.sextantGlobalPiece <= GLOBAL_LIMIT_LEFT
	...
	private static void moveSextantGlobalPieceWithFormula(Player c) {
		int degree = c.sextantGlobalPiece < -GLOBAL_LIMIT_RIGHT ? -GLOBAL_LIMIT_RIGHT
				: c.sextantGlobalPiece > GLOBAL_LIMIT_LEFT ? GLOBAL_LIMIT_LEFT
						: c.sextantGlobalPiece;
		int realDegree = degree > 0 ? degree : 2047 - (-degree);
		c.getPacketSender().sendInterfaceEdit(SEXTANT_TELESCOPE, SEXTANT_ROTATION1,
				realDegree, SEXTANT_GLOBAL_ZOOM);
		c.getPacketSender().sendInterfaceEdit(SEXTANT_ARC, SEXTANT_ROTATION1,
				realDegree, SEXTANT_GLOBAL_ZOOM);
	...
	public static void moveLandScapeWithFormula(Player c) {
		c.getPacketSender().sendInterfaceComponentMoval(0, c.sextantLandScapeCoords,
				SEXTANT_LANDSCAPE);
	}
	...
	public static void moveSunWithFormula(Player c) {
		c.getPacketSender().sendInterfaceComponentMoval(0, c.sextantSunCoords, SEXTANT_SUN);
	}

	/* display the player position in degree and minutes */
	public static void displayPosition(Player c) {
		String resultX = CoordinateScrolls.calculateActualPosition(c
				.getPosition().getX(), c.getPosition().getY())[0];
		String resultY = CoordinateScrolls.calculateActualPosition(c
				.getPosition().getX(), c.getPosition().getY())[1];

		c.getPacketSender().sendMessage("the sextant displays:");
		c.getPacketSender().sendMessage(resultX);
		c.getPacketSender().sendMessage(resultY);
	}
	...
	public static boolean handleSextantButtons(Player c, int buttonId) {
		...
		case 6952:
			...
			return true;
		case 6959: // Sextant give Location
