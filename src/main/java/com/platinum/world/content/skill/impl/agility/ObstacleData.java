package com.platinum.world.content.skill.impl.agility;
...
public enum ObstacleData {
	...
	ROPESWING_LADDER(1759, false, false) {
		@Override
		public void cross(final Player player) {
			player.performAnimation(new Animation(827));
	...
	STEPPING_STONES(9326, true, true) {
		@Override
		public void cross(final Player player) {
			player.getUpdateFlag().flag(Flag.APPEARANCE);
				...
				public void execute() {
					...
					if (tick == 4 || tick == 7 || tick == 10 || tick == 13 || tick == 16) {
						...
						if (player.getLocation() == Location.WILDERNESS) {
							...
							if (player.getPosition().getX() <= 2770 && player.getLocation() != Location.WILDERNESS) {
								player.moveTo(new Position(2769, 10002, 0));
								Agility.addExperience(player, 250);
								stop();
								return;
							} else if (player.getPosition().getX() >= 2773 && player.getLocation() != Location.WILDERNESS) {
	...
	BALANCE_LEDGE_2(2297, true, true) {
	...
	/** MISC **/
	RED_DRAGON_LOG_BALANCE(5088, false, true) {
	...
	ObstacleData(int object, boolean mustWalk, boolean wild) {
		this.object = object;
