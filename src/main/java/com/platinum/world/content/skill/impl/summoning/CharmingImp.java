...
public class CharmingImp {
	...
	public static void sendConfig(Player player) {
		for(int i = 0; i < 4; i++) {
			...
			switch(state) {
			case 0:
				player.getPacketSender().sendMessage("<img=10> <col=996633>Your Looting Bag is placing all "+ItemDefinition.forId(charm).getName()+"s it finds in your inventory.");
				break;
			case 1:
				player.getPacketSender().sendMessage("<img=10> <col=996633>Your Looting Bag is turning all "+ItemDefinition.forId(charm).getName()+"s it finds into Summoning exp.");
				break;
			}
		}
	}
