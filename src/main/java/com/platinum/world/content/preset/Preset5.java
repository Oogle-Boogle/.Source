package com.platinum.world.content.preset;
...
public class Preset5 {
	...
	public static void load(Player p) {
		...
		if (p.presetItems5 == null) {
			return;
		}
		if (!Player.preset5) {
			p.getPacketSender().sendMessage("You don't have a loadout in preset 5.");
			return;
		}
		p.getPacketSender().sendMessage("You have loaded Preset 5.");
		...
		for (int j = 0; j < 56; j += 2) {
			Item inv = new Item(p.presetItems5[j], p.presetItems5[j + 1]);
			int tab = Bank.getTabForItem(p, p.presetItems5[j]);
			if (p.getBank(tab).contains(inv.getId(), inv.getAmount())) {
				p.getBank(tab).delete(p.presetItems5[j], p.presetItems5[j + 1]);
				p.getInventory().add(p.presetItems5[j], p.presetItems5[j + 1]);
			} else {
				p.getPacketSender().sendMessage("You are missing " + inv.getDefinition().getName());
			}

		}
		Item head = new Item(p.presetGear5[0], p.presetGear5[1]);
		Item cape = new Item(p.presetGear5[2], p.presetGear5[3]);
		Item amulet = new Item(p.presetGear5[4], p.presetGear5[5]);
		Item weapon = new Item(p.presetGear5[6], p.presetGear5[7]);
		Item body = new Item(p.presetGear5[8], p.presetGear5[9]);
		Item shield = new Item(p.presetGear5[10], p.presetGear5[11]);
		Item leg = new Item(p.presetGear5[14], p.presetGear5[15]);
		Item hands = new Item(p.presetGear5[18], p.presetGear5[19]);
		Item feet = new Item(p.presetGear5[20], p.presetGear5[21]);
		Item ring = new Item(p.presetGear5[24], p.presetGear5[25]);
		Item amunnution = new Item(p.presetGear5[26], p.presetGear5[27]);
	...
	public static void savingInv(Player p) {
		...
		p.presetItems5 = new int[] { inv0.getId(), inv0.getAmount(), inv1.getId(), inv1.getAmount(), inv2.getId(),
	...
	public static void savingGear(Player p) {
		...
		p.presetGear5 = new int[] { inv0.getId(), inv0.getAmount(), inv1.getId(), inv1.getAmount(), inv2.getId(),
				inv2.getAmount(), inv3.getId(), inv3.getAmount(), inv4.getId(), inv4.getAmount(), inv5.getId(),
				inv5.getAmount(), inv6.getId(), inv6.getAmount(), inv7.getId(), inv7.getAmount(), inv8.getId(),
				inv8.getAmount(), inv9.getId(), inv9.getAmount(), inv10.getId(), inv10.getAmount(), inv11.getId(),
