package com.platinum.world.entity.updating;
...
import com.platinum.model.Gender;
import com.platinum.model.Position;
import com.platinum.model.Skill;
import com.platinum.model.UpdateFlag;
...
import com.platinum.net.packet.PacketBuilder.AccessType;
...
public class PlayerUpdating {
	...
	private static void updateChat(PacketBuilder builder, Player target) {
		...
		builder.put(target.getSecondaryPlayerRights().ordinal());
		builder.put(target.isGim() ? 3 : target.getGameMode().ordinal());
	...
	private static void updateAppearance(Player player, PacketBuilder out, Player target) {
		...
		if (player.getNpcTransformationId() <= 0) {
			...
			if (equip[Equipment.HEAD_SLOT] > -1) {
				...
				/*if (equip[Equipment.HEAD_SLOT] == 1050) {
					int[] santaColors = target.getSantaColors();
					if (santaColors != null) {
						properties.put(santaColors.length);
						for (int i = 0; i < santaColors.length; i++) {
							properties.putInt(santaColors[i]);
			...
			if (equip[Equipment.CAPE_SLOT] > -1) {
				...
				if (equip[Equipment.CAPE_SLOT] == 14019 || equip[Equipment.CAPE_SLOT] == 14022) {
					int[] modelColors = equip[Equipment.CAPE_SLOT] == 14022 ? target.getCompCapeColors()
							: target.getMaxCapeColors();
					if (modelColors != null) {
